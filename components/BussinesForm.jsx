...
export default function BussinesForm() {
  ...
  const submitBusinessForm = async (data) => {
    try {
      ...
      const user = JSON.parse(
        Buffer(localStorage.getItem("user"), "base64").toString("ascii")
      );
      const id = user._id;
      ...
      if (response.status === 200) {
        localStorage.setItem(
          "user",
          Buffer(JSON.stringify(dataJson.user)).toString("base64")
        );
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      setRfc(user.rfc);
      setLegalRepresentative(user.legalRepresentative);
      setType(user.type);
    ...
    return (
      <>
        <form
          ref={formRef}
          className={clsx(
            "absolute flex flex-col rounded-[16px] border-2 border-blue-sky-50 p-5 mr-10 max-lg:mr-[0px] shadow-xl w-[700px] max-lg:w-[340px]"
          )}
        ...
        >
          ...
          <section className={clsx("max-lg:inline-block")}>
            ...
            <section
              className={clsx(
                "max-lg:inline-block flex grid-rows-2 justify-between"
              )}
            >
              <article>
                <Input
            ...
            <div style={{ display: showFormExpanded ? "block" : "none" }}>
              ...
              <div id="minimap-container" className="mt-2  w-[300px] h-[200px]">
                <AddressMinimap
                  canAdjustMarker
                  satelliteToggle
                  feature={feature}
                ...
                {showFormExpanded && (
                  <div className="flex justify-end w-[300px] h-[25px] mt-20">
                    <button
                      type="button"
                      className={clsx(
                        "px-3 border border-slate-300 rounded-md",
